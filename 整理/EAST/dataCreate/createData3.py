...
def drawText(img, target, yx, fonts, gridSize, gridColor, maxStrLen=13, textOrientation=random.randint(0, 2)):
	...
	for char in text:
		...
		if(nextX + twidth > endX):
			boxs.append([baseX, nextY
						,nextX, nextY
						,nextX, nextY+textheight
						,baseX, nextY+textheight, txtHeap])
			txtHeap = ""
			nextX = baseX
			nextY += textheight
	...
	for box in boxs:
		box[0], box[1] = get_point(box[0], box[1], (baseX+endX)//2, (baseY+endY)//2, angle)
		box[2], box[3] = get_point(box[2], box[3], (baseX+endX)//2, (baseY+endY)//2, angle)
		box[4], box[5] = get_point(box[4], box[5], (baseX+endX)//2, (baseY+endY)//2, angle)
		box[6], box[7] = get_point(box[6], box[7], (baseX+endX)//2, (baseY+endY)//2, angle)
...
def createImg(fonts):
	...
	gridSize = random.randint(80, 600)  #網格大小
...
if __name__ == '__main__' : 
	pathImgStore = "./train_img/"
	pathTargetStore = "./train_mask/"
	pathTargetStoreTxt = "./train_gt/"
	maxImgCount = 20000
	fonts = loadFonts()
