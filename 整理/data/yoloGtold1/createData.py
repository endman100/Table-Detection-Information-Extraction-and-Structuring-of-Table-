...
def drawText(img, target, yx, fonts, gridSize, gridColor, maxStrLen=13, textOrientation=random.randint(0, 2)):
	...
	x1 = max(min(returnBox[0], returnBox[2], returnBox[4], returnBox[6]), baseX)
	x2 = min(max(returnBox[0], returnBox[2], returnBox[4], returnBox[6]), endX)
	y1 = max(min(returnBox[1], returnBox[3], returnBox[5], returnBox[7]), baseY)
	y2 = min(max(returnBox[1], returnBox[3], returnBox[5], returnBox[7]), endY)
...
def createImg(fonts):
	imgSizeHeight = 608#random.randint(800, 1500)  #隨機的圖片高度
	imgSizeWidth = 608#random.randint(800, 1500)  #隨機的圖片寬度
...
if __name__ == '__main__' : 
	...
	for i in range(1, maxImgCount+1):
		...
		data = []
		for box in boxs:
			starth, startw = max(min(box[1::2]), 0), max(min(box[::2]), 0)
			endh, endw = min(max(box[1::2]), h), min(max(box[::2]), w)
			centerh, centerw = (endh + starth)/2/h, (endw + startw)/2/w
			boxh, boxw =  (endh - starth)/2/h, (endw - startw)/2/w
			box = [0, centerw, centerh, boxw, boxh]
			data.append(" ".join([str(num) for num in box]) + "\n")
		# print(data)
		with open(pathImgStore + str(i)+ ".txt", "w", encoding='UTF-8') as f:
			f.writelines(data)